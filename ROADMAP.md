...
- 已补齐证据链：每条建议同时挂接错题样本与 FSRS 到期证据，保证建议可解释。
- 已新增 7/14/30 天对比面板：关键行为指标可横向比较窗口趋势。
- 已补齐报告结构：导出区域包含“计划 vs 完成”（动作数与时长完成度）。
- 已新增 Phase 3 验收指标卡：建议执行率目标（>=40%）与家长面板周活提升目标（+20%）支持自动判定。
...
### 当前进展（2026-02-08）
- 已落地一致性巡检 v0：对 `learningEvents / playerProfile / history` 建立自动对账检查，并在家长面板展示告警状态与偏差值。
- 已落地 AI 稳定性监控 v0：OpenRouter 请求重试/限速命中/延迟指标落库，并在家长面板提供成功率与非限速比例目标判定。
- 已落地会话恢复监控 v0：恢复尝试/成功/失败事件落库，面板展示 14 天恢复成功率与达标状态。
...
学习科学、游戏化与体验方向已完成专项审计，详见 [`docs/LEARNING_SCIENCE_UX_ROADMAP.md`](docs/LEARNING_SCIENCE_UX_ROADMAP.md)。M0–M3 已完成；M4 仓库内的证据校准与试点准备实现见 [`docs/M4_EVIDENCE_CALIBRATED_PILOT.md`](docs/M4_EVIDENCE_CALIBRATED_PILOT.md)。

### M4：证据校准与真实试点准备（仓库内实现完成）

- Objective Registry v2 固定目标版本、别名、知识成分、先修与最低证据要求；未知目标隔离，不更新掌握度。
- 题目族、等价组、测量角色、迁移距离、审核状态和证据强度贯穿题目缓存、FSRS、学习事件与掌握记录。
- 普通练习、独立提取、24h/7d 保持与新语境迁移分开计量；即时修复和未审核题不能伪装成强证据。
- 受控本地内容包、保持探测、失败修复/冷却重测、成人审核队列与 v14→v15 保守迁移均已实现并测试。
- 家长面板改为“保持目标 + 合格样本”，低样本隐藏分数；导出不再把技能练习正确率称为掌握度。
- 游戏化改为证据校准奖励和非惩罚性活跃学习日；反馈增加答案、原因与可迁移策略，并尊重 reduced-motion。

真实学习者试点仍未执行。启动前必须单独获得适用的隐私/伦理审查、监护人知情同意与学习者同意/认可，并冻结指标和退出规则；不得把自动化测试、教师走查或合成数据描述为学习成效。
