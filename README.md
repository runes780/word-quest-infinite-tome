...
| Offline recovery and local persistence | Implemented | Dexie/IndexedDB schema v16 stores learning evidence, task-contract snapshots, and local content reviews. Versioned backups transactionally cover every table; migration tests preserve older optional-field records, conservatively downgrade legacy unsupported mastery, and reject future/corrupt files before writes. No cloud sync is implemented. |
