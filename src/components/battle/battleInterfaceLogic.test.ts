...
describe('battle interface orchestration rules', () => {
    ...
    test('surfaces Lucky rewards without hiding repair guidance', () => {
        const result = { explanation: 'Good work.', repairQueued: true, isLucky: true };

        expect(formatBattleResultExplanation(result, 'en')).toContain('Lucky! +50% rewards');
        expect(formatBattleResultExplanation(result, 'en')).toContain('same-pattern repair question');
        expect(formatBattleResultExplanation(result, 'zh')).toContain('幸运！奖励 +50%');
    });
