...
import { applyBuildVerb, applyUnlockedVerbs } from '@/lib/data/verbProgression';
...
export const createLearningSlice: StateCreator<GameState, [], [], LearningSlice> = (set, get) => ({
    ...
    addQuestions: (newQuestions) => {
        ...
        const withListen = applyUnlockedVerbs(processedQuestions, get().unlockedVerbs);
        const withUnlockedVerbs = applyBuildVerb(withListen, get().unlockedVerbs);
        set({ questions: [...questions, ...withUnlockedVerbs] });
    },
